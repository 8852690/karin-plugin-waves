 ...
 * @param {karin.Event} e
 */
const towerReg = /^(?:～|~|∽|#?鸣潮)(?:逆境)?(?:深[塔渊]|(稳定|实验|超载|深境)区?)(\d{9})?$/;
export const Tower = karin.command(towerReg, async (e) => {
    const user = e.at.shift() || e.user_id;

    // 匹配用户消息, 获取深渊名和可能存在的特征码.
    const [tower, uid] = e.msg.match(towerReg).slice(1);
    ...
    for (const acc of accountList) {
        ...
        const [ baseData, towerData ] = await Promise.all([
            ...
            waves.getTowerData(acc.serverId, acc.roleId, acc.token),
        ]);

        // 检查是否获取到数据.
        if (!baseData.status || !towerData.status) {
            data.push(`[UID: ${acc.roleId}] 获取深渊数据失败, 原因: ${baseData.msg || towerData.msg}`);
        }
        else {
            // 匹配对应的深渊数据, 若匹配数据为空, 则返回默认数据.
            const towerMap = { '稳定': 1, '实验': 2, '深境': 3, '超载': 4 };
            const towerKey = tower ? towerMap[tower] : 3;

            // 检查是否有对应的深渊数据.
            if (!towerData.data.difficultyList.some(({ difficulty, towerAreaList }) => difficulty === towerKey && towerAreaList.length)) {
                data.push(`[UID: ${acc.roleId}] 未找到 ${tower || '深境'} 区域数据.`);
                continue;
            }

            // 渲染深渊数据.
            const img = await Render.render('Template/towerData/tower', {
                avatarUrl: (!uid && await redis.get(`karin:waves:users:${user}`)) ? e.bot.getAvatarUrl(user) : '',
                baseData: baseData.data,
                towerData: { ...towerData.data, difficulty: towerKey, diffiname: tower || '深境' },
            });
    ...
    const msg = data.length === 1
        ? data[0]
        : common.makeForward(
            [segment.text(`[用户: ${user}] 深渊查询结果:`), ...data],
            e.bot.account.uin,
            e.bot.account.name,
        );
