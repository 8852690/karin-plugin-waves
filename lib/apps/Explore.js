 ...
 * @param {karin.Event} e
 */
const exploreReg = /^(?:～|~|∽|#?鸣潮)(?:探索度?|地图)(\d{9})?$/;
export const Explore = karin.command(exploreReg, async (e) => {
    const user = e.at.shift() || e.user_id;

    // 匹配用户消息, 获取深渊名和可能存在的特征码.
    const uid = e.msg.match(exploreReg)[1];
    ...
    for (const acc of accountList) {
        ...
        const [ baseData, exploreData ] = await Promise.all([
            ...
            waves.getExploreData(acc.serverId, acc.roleId, acc.token),
        ]);

        // 检查是否获取到数据.
        if (!baseData.status || !exploreData.status) {
            data.push(`[UID: ${acc.roleId}] 未获取到探索数据, 原因: ${baseData.msg || exploreData.msg}`);
        }
        else {
            // 渲染探索数据.
            const img = await Render.render('Template/exploreIndex/exploreIndex', {
    ...
    const msg = data.length === 1
        ? data[0]
        : common.makeForward(
            [segment.text(`[用户: ${user}] 探索查询结果:`), ...data],
            e.bot.account.uin,
            e.bot.account.name,
        );
