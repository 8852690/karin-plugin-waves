...
import { resPath, basename } from './dir.js';

class WeightCalculator {
    constructor(roleDetail) {
        this.roleDetail = roleDetail;
        this.roleWeightPath = `${resPath}/Weight/${roleDetail.role.roleId}.yaml`;
        this.baseWeightPath = `${resPath}/Weight/weight.yaml`;
    }
        ...
        const calMainProps = (COST) => {
            phantom.mainProps.forEach(tag => {
                ...
                try {
                    ...
                    logger.mark(logger.blue(`[${basename}]`), logger.cyan(`疑似该声骸属性异常`), logger.red(JSON.stringify(phantom)));
                }
            });
        };
    ...
    gatherTags(phantomList, roleWeight) {
        ...
        const totalScore = phantomList.reduce((sum, phantom) => {
